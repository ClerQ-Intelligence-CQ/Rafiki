...
impl WelfordDecayed {
    pub fn new() -> Self {
        Self { n_eff: 0.0, mean: 0.0, m2: 0.0, tau: DECAY_TAU }
    }
...
impl CategoryDist {
    ...
    pub fn leader(&self) -> Option<String> {
        self.counts.iter().max_by(|a, b| a.1.partial_cmp(b.1).unwrap()).map(|(k, _)| k.clone())
    }
}
...
impl TambuaEngine {
    ...
    pub fn absorb_features(&mut self, feats: &[Feature], out: &mut Vec<BaselineUpdate>) {
        for f in feats {
            ...
            if (mean - entry.1).abs() > 0.25 * std.max(1e-6) || stability_band(stab) != entry.2 {
                ...
                self.publishes += 1;
                out.push(BaselineUpdate { feature: f.name.to_string(), mean, stability: stab });
            }
    ...
    pub fn absorb_classes(&mut self, classes: &[Classification], out: &mut Vec<BaselineUpdate>) {
        for c in classes {
            let dist = self.categories.entry(c.kind.to_string()).or_insert_with(|| CategoryDist {
                counts: HashMap::new(),
                tau: DECAY_TAU,
            });
    ...
    pub fn footprint_tracks(&self) -> usize {
        self.numerics.len() + self.categories.values().map(|d| d.counts.len()).sum::<usize>()
    }
}
...
mod tests {
    ...
    fn welford_matches_batch_statistics() {
        ...
        let mut w = WelfordDecayed { n_eff: 0.0, mean: 0.0, m2: 0.0, tau: 1e9 };
        for &x in &xs {
            w.observe(x);
        }
        let n = xs.len() as f32;
    ...
    fn engine_absorbs_and_reports() {
        ...
        let mut out = Vec::new();
        let feats = vec![
            Feature { name: "a", value: 1.0, confidence: 1.0, completeness: 1.0 },
            Feature { name: "b", value: 2.0, confidence: 1.0, completeness: 1.0 },
        ];
        for _ in 0..5000 {
