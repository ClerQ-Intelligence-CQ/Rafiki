//! Fikiri fork rebenchmark: full SAE to SPE to Fikiri chain on
...
//! Run: cargo run --release -p rafiki-fikiri --example bench
...
fn main() {
    ...
    for i in 0..total {
        ...
        for (ty, d) in [
            ...
            if let Some((feats, classes)) = spe.ingest(&e) {
                let mut out = Vec::new();
                fikiri.absorb_features(&feats, &mut out);
                fikiri.absorb_classes(&classes, &mut out);
                for c in &classes {
        ...
        if i == 1599 {
            mic_before = fikiri.numeric_mean("mic.env_mean").unwrap_or(f32::NAN);
            numerics_half = fikiri.numerics_len();
        }
    }
    let elapsed = t0.elapsed();
    let mic_after = fikiri.numeric_mean("mic.env_mean").unwrap_or(f32::NAN);
    ...
    let tracks = fikiri.footprint_tracks();
    ...
    println!(
        ...
        fikiri.updates_absorbed, fikiri.publishes
    );
    println!(
    ...
    if fikiri.numerics_len() != numerics_half {
    ...
    if failures.is_empty() {
        println!("FIKIRI FORK VALIDATION: PASS");
    } else {
        println!("FIKIRI FORK VALIDATION: FAIL");
        for f in &failures {
            println!("  - {f}");
        }
        std::process::exit(1);
